...
use TypedPatternEngine\Heuristic\Analyzer\NodeAnalyzer;
...
final class PatternHeuristic implements HeuristicPatternInterface
{
     ...
     * @param int $minLen
     * @param int $maxLen
     * @param array<string, true> $requiredLiterals
     * @param string|null $commonPrefix
     * @param string|null $commonSuffix
     * @param array<string, true> $possiblePrefixes
     * @param array<string, true> $possibleSuffixes
     * @param array<int, true> $allowedChars
     * @param bool $hasChecks
     */
    ...
    public function support(string $string): bool
    {
        ...
        if (!empty($this->allowedChars)) {
            for ($i = 0; $i < $len; $i++) {
                if (!($this->allowedChars[ord($string[$i])] ?? false)) {
    ...
    public function getStats(): array
    {
        return [
            ...
            'allowedCharsCount' => empty($this->allowedChars) ? 'all' : count($this->allowedChars),
            'hasChecks' => $this->hasChecks
        ];
    }
}
